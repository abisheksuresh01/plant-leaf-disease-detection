...
def read_image(fname, dim=(32, 32)):
    img = cv2.imread(fname)
    return cv2.resize(img, dim)

def generate_data():
    num = 10000000
    pos_lines = open('healthy.txt', 'r').readlines()[:num]
    neg_lines = open('unhealthy.txt', 'r').readlines()[:num]
    pos = list(map(lambda x: read_image(x.strip()), pos_lines))
    neg = list(map(lambda x: read_image(x.strip()), neg_lines))
    random.seed(84829374695043)
    random.shuffle(pos)
    random.shuffle(neg)
    train_count = int(0.8 * len(pos))
    pos_train, pos_test = pos[:train_count], pos[train_count:]
    neg_train, neg_test = neg[:train_count], neg[train_count:]
...
checkpointer = ModelCheckpoint('models/best_model.h5', monitor='val_accuracy', verbose=1, save_best_only=True,
                               save_weights_only=True)

training = model.fit(x_train, y_train, epochs=50, batch_size=32, verbose=1,
                     callbacks=[earlystopper, reduce_lr, checkpointer], validation_data=(x_test, y_test))
